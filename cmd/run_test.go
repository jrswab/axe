...
func TestRun_ArtifactDirFlagCreatesEnvVar(t *testing.T) {
	resetRunCmd(t)
	server := startMockAnthropicServer(t)
	defer server.Close()
	...
	setupRunTestAgent(t, "artifact-env-agent", `name = "artifact-env-agent"
model = "anthropic/claude-sonnet-4-20250514"
`)
	t.Setenv("ANTHROPIC_API_KEY", "test-key")
	t.Setenv("AXE_ANTHROPIC_BASE_URL", server.URL)
	...
	rootCmd.SetOut(buf)
	rootCmd.SetErr(errBuf)
	rootCmd.SetArgs([]string{"run", "artifact-env-agent", "--artifact-dir", artifactDir})
	...
	// Verify AXE_ARTIFACT_DIR was set
	if os.Getenv("AXE_ARTIFACT_DIR") != artifactDir {
		t.Errorf("expected AXE_ARTIFACT_DIR to be set to %q, got %q", artifactDir, os.Getenv("AXE_ARTIFACT_DIR"))
	}
...
func TestRun_ArtifactsIncludedInJSON_WhenActive(t *testing.T) {
	resetRunCmd(t)

	artifactDir := t.TempDir()

	// Two responses: first a tool call with artifact flag, then final text
	server := testutil.NewMockLLMServer(t, []testutil.MockLLMResponse{
	...
	tmpDir := setupRunTestAgent(t, "json-artifact-agent-active", `name = "json-artifact-agent-active"
model = "anthropic/claude-sonnet-4-20250514"
tools = ["write_file"]
...
`)
	...
	rootCmd.SetOut(buf)
	rootCmd.SetErr(errBuf)
	rootCmd.SetArgs([]string{"run", "json-artifact-agent-active", "--json"})
	...
	if !ok {
		t.Fatalf("expected 'artifacts' key in JSON output, got keys: %v", getKeys(result))
	}
	...
	if len(artifacts) == 0 {
		t.Fatalf("expected 'artifacts' array to have at least one entry, got empty array")
	}

	// Assert first entry has required keys: path, agent, size
	firstArtifact, ok := artifacts[0].(map[string]interface{})
	if !ok {
		t.Fatalf("expected artifact to be an object, got %T", artifacts[0])
	}

	for _, key := range []string{"path", "agent", "size"} {
		if _, ok := firstArtifact[key]; !ok {
			t.Errorf("expected artifact to have '%s' key", key)
		}
	}
}

func TestRun_ArtifactsAbsentInJSON_WhenInactive(t *testing.T) {
	resetRunCmd(t)
	...
	tmpDir := setupRunTestAgent(t, "json-artifact-agent-inactive", `name = "json-artifact-agent-inactive"
model = "anthropic/claude-sonnet-4-20250514"
`)
	t.Setenv("XDG_CONFIG_HOME", tmpDir)
	t.Setenv("ANTHROPIC_API_KEY", "test-key")
	...
	rootCmd.SetOut(buf)
	rootCmd.SetErr(errBuf)
	rootCmd.SetArgs([]string{"run", "json-artifact-agent-inactive", "--json"})
	...
	// Assert "artifacts" key does NOT exist when artifacts are inactive
	if _, ok := result["artifacts"]; ok {
		t.Errorf("expected 'artifacts' key to be absent when artifacts are inactive, but it was present: %v", result["artifacts"])
	}
}
...
func TestRun_ArtifactsTOMLDirSetting(t *testing.T) {
	resetRunCmd(t)
	server := startMockAnthropicServer(t)
	defer server.Close()
	...
	// Agent with TOML artifacts.dir setting
	setupRunTestAgent(t, "toml-artifact-agent", `name = "toml-artifact-agent"
model = "anthropic/claude-sonnet-4-20250514"
...
`)
	...
	rootCmd.SetOut(buf)
	rootCmd.SetErr(errBuf)
	rootCmd.SetArgs([]string{"run", "toml-artifact-agent"})
	...
	// Verify AXE_ARTIFACT_DIR was set to the TOML directory
	if os.Getenv("AXE_ARTIFACT_DIR") != artifactDir {
		t.Errorf("expected AXE_ARTIFACT_DIR to be set to TOML dir %q, got %q", artifactDir, os.Getenv("AXE_ARTIFACT_DIR"))
	}

	// Clean up env var
	_ = os.Unsetenv("AXE_ARTIFACT_DIR")

	// Verify the data directory was created (since it's a persistent dir from TOML)
	if _, err := os.Stat(artifactDir); os.IsNotExist(err) {
		t.Errorf("expected TOML artifact directory to exist, but it does not")
	}
}

func TestRun_ArtifactsDisabledByDefault(t *testing.T) {
	resetRunCmd(t)
	server := startMockAnthropicServer(t)
	defer server.Close()

	// Agent without artifacts configuration
	setupRunTestAgent(t, "no-artifact-agent", `name = "no-artifact-agent"
model = "anthropic/claude-sonnet-4-20250514"
`)
	t.Setenv("ANTHROPIC_API_KEY", "test-key")
	t.Setenv("AXE_ANTHROPIC_BASE_URL", server.URL)
	...
	rootCmd.SetOut(buf)
	rootCmd.SetErr(errBuf)
	rootCmd.SetArgs([]string{"run", "no-artifact-agent"})
	...
	// Verify AXE_ARTIFACT_DIR was NOT set (artifacts disabled by default)
	if os.Getenv("AXE_ARTIFACT_DIR") != "" {
		t.Errorf("expected AXE_ARTIFACT_DIR to be unset when artifacts disabled, got %q", os.Getenv("AXE_ARTIFACT_DIR"))
	}
}
